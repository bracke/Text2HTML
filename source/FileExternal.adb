...
package body FileExternal is
         ...
         if Itemtype = 1 then
            ...
            if Recursive then
               Nr:= Nr + Nr_Of_Files(Path & "." &
                                     to_String(Itemname),true,Count_Dirs);
            end if;
         end if;
      end loop Scan_Dir;
      return Nr;
   ...
   function Get_Directory_List (Path       : in String;
      ...
      Path_Expanded : String       := FileName.Convert_Path(Path);
      Files     : Natural          := Nr_Of_Files(Path_Expanded,false,Count_Dirs);
   ...
   begin
      while Index /= -1 loop
         Name := U("");
         Read_Dir(Path_Expanded,Index,Name);

         if Ada.Strings.Unbounded.Length(Name) > 0 then
            Object_Type := Get_Object_Type(S(Path_Expanded & U(".") & Name));
   ...
   procedure Get_File_Information (Filename   : in string;
                                   Loadadr    : out integer;
                                   Execadr    : out integer;
                                   Length     : out integer;
                                   Attributes : out integer;
   ...
   begin
      ...
      if Register.R(0) = 0 then
        return Not_Found;
      elsif Register.R(0) = 1 then
        return File_Object;
      elsif Register.R(0) = 2 then
        return Dir;
      elsif Register.R(0) = 3 then
        return Image;
      else
        return Not_Found;
      end if;
