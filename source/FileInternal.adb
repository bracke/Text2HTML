...
package body FileInternal is
   ...
   procedure GotoEnd (File : in FileHandle_Type) is
   begin
      Set_Ptr(File,Get_Size(File));
   end GotoEnd;

   --

   function IsEOF (File : in FileHandle_Type) return boolean is
   ...
   begin   
     ...
     if Error /= null then
        pragma Debug(Reporter.Report("FileInternal.IsEOF: " & To_Ada(Error.ErrMess)));
   ...
   function Get_Size (File : in FileHandle_Type) return natural is
   ...
   begin
     ...
     if Error /= null then
        pragma Debug(Reporter.Report("FileInternal.Get_Size: " & To_Ada(Error.ErrMess)));
        OS.Raise_Error(Error);
        raise File_Error;
     end if;
   ...
   function Read_Line (File : in FileHandle_Type) return String is
   ...
   procedure Load_File (FileName : in String;
      ...
      FileName_0 : string := FileName & Character'Val(0);
   ...
   function Get_Real_FileHandle (File : in FileHandle_Type) return Real_FileHandle_Type is   
   begin
      return File.Handle;
